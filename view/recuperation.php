   ...
   if(!empty($_POST['recup_mail'])) {
      ...
      if(filter_var($recup_mail,FILTER_VALIDATE_EMAIL)) {
         ...
         mail($recup_mail, "Récupération de mot de passe - Votresite", $message, $header);
            header("Location:http://127.0.0.1/recuperation.php?section=code");
         } else {
...
if(isset($_POST['verif_submit'],$_POST['verif_code'])) {
   if(!empty($_POST['verif_code'])) {
      ...
      if($verif_req == 1) {
         ...
         header('Location:http://127.0.0.1/recuperation.php?section=changemdp');
      } else {
         $error = "Code invalide";
      }
   } else {
         ...
         if(!empty($mdp) AND !empty($mdpc)) {
            if($mdp == $mdpc) {
              ...
              $del_req->execute(array($_SESSION['recup_mail']));
               header('Location:http://127.0.0.1/connexion/');
            } else {
