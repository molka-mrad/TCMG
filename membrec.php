...
class membrec
{
	...
	{
		$sql = "insert into membres (cin,nom,prenom,tel,age,inscription,abonnement,email,mdp,paiement,role) values (:cin, :nom,:prenom,:tel,:age,:inscription,:abonnement,:email,:mdp,:paiement,:role)";
		$db = config::getConnexion();
		try {
			$req = $db->prepare($sql);
	...
	{
		$sql = "UPDATE membres SET cin=:cinn, nom=:nom,prenom=:prenom,tel=:tel,age=:age,inscription=:inscription,abonnement=:abonnement,paiement=:pay,role=:role,email=:malo,mdp=:mdp WHERE cin=:cin";
